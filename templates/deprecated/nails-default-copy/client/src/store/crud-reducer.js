const crudReducer = (state, action) => {
  ...
  switch (action.type_action) {
    ...
    case "UPDATE":
      ...
      let indexUpdate = state[endpoint].findIndex(obj => obj._id === action.response.data._id)
    ...
    case "DESTROY":
      let indexDelete = state[endpoint].findIndex(obj => obj._id === action.response.data._id)
