...
// Template for adding timestamps to a model
const timestamps = {
  created_at: { type: "date", default: "date.now" },
  updated_at: { type: "date", default: "date.now" }
};

// Runs async functions synchronously. Can also take an object of functions. Must add function obj key on each array item if passing in object functions.
...
// Covers the logic for determining what migations/rollbacks to perform. I.E. waht has already been rolled back or migrated.
    ...
    if (applied) {
      if (applied_migrations[file]) {
        appliedMigrations.push({
          ...
          batch: applied_migrations[file].batchDate
        });
      }
    }
    if (pending) {
